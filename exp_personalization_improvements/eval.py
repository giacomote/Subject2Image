...
def test_subject_driven_pipeline(
    ...
    3. Computing average CLIP-I, CLIP-T and DINO-I on the generated images
    ...
    # Preliminary cleanup of the adaptation folder to reset the previous weights
    if os.path.exists(adaptation_dir):
        shutil.rmtree(adaptation_dir)
    ...
    subject_gen_dir = os.path.join(generation_dir, subject_name)
    ...
    for gen_img, eval_prompt in generated_samples:
        score_t = evaluator.compute_clip_t(gen_img, eval_prompt)
...
if __name__ == '__main__':
    # Clean recreation of the evaluation folder at each run
    if os.path.exists(EvaluationConfig.generation_dir):
        shutil.rmtree(EvaluationConfig.generation_dir)
    os.makedirs(EvaluationConfig.generation_dir, exist_ok=True)
    ...
    # Synchronized iteration based on the configured subject names
    for subject_idx, cfg in enumerate(EvaluationConfig.subject_cfgs):
        subject_name = cfg['name']
        data_dir = os.path.join(EvaluationConfig.data_dir, subject_name)

        # Check that the configured data folder exists 
        assert os.path.isdir(data_dir), (f'[ERROR] Data folder for subject \'{subject_name}\' not found')
        ...
        sub_clip_t, sub_clip_i, sub_dino_i = test_subject_driven_pipeline(
            ...
            token_identifier=cfg['token_identifier'],
            data_dir=data_dir,
