...
from pipeline_ti.config.evaluation_config import EvaluationConfig
...
def train_and_generate_for_subject(
    ...
    print(f'[TG 1/2] Textual Inversion...\n')
    pipe = BaseTIPipe()
    ...
    embeds_path = os.path.join(adaptation_dir, 'ti_learned_embeds.safetensors')
    assert os.path.exists(embeds_path), f'({embeds_path}) No learned embeddings found'
    ...
    for p_idx, prompt in enumerate(test_prompts):
        for s_idx in range(samples_per_prompt):
            ...
            pipe.generate_personalized_image_ti(
                embeds_dir=adaptation_dir,
